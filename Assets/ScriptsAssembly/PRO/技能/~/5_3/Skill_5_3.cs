...
namespace GamePlay
{
    public class Skill_5_3 : OperateFSMBase, IOperate_射线选择
    {
        ...
        {
            AddState(new Skill_5_3_T0());
            AddState(new Skill_5_3_T1());
            AddState(new Skill_5_3_T2());
        }

        public class Skill_5_3_T0 : OperateStateBase_T0
        {
            public override OperateFSMBase Operate { get => operate; set => operate = (Skill_5_3)value; }
            private Skill_5_3 operate;
        ...
        public class Skill_5_3_T1 : OperateStateBase_T1
        {
            public override OperateFSMBase Operate { get => operate; set => operate = (Skill_5_3)value; }
            private Skill_5_3 operate;
        ...
        public class Skill_5_3_T2 : OperateStateBase_T2
        {
            public override OperateFSMBase Operate { get => operate; set => operate = (Skill_5_3)value; }
            private Skill_5_3 operate;
            private float time;
            public override void Enter()
            {
                ...
                particle.Renderer.color = BlockMaterial.GetPixelColorInfo("带电空气色").color;
                ...
                particle.UpdateEvent += (p) =>
                {
                    ...
                    if (pixel != null && (pixel.typeInfo.tags.Contains("导电") || pixel.typeInfo.tags.Contains("带电")))
                    {
                        operate.context.ClearByAgentData();
                        RaycastHit2D[] hits = Physics2D.CircleCastAll(p.transform.position, 30 * Pixel.Size, Vector2.zero, 0, 1 << (int)GameLayer.Role | 1 << (int)GameLayer.UnRole);
                        int index = 0;
                        foreach (var hit in hits)
                        {
                            var nextRole = GamePlayMain.Inst.GetRole(hit.transform);
                            var nextRole_buff = nextRole.GetBuff<Buff_2_5>();
                            if (nextRole_buff != null && nextRole_buff.GetActive())
                            {
                                BuffEx.Set导电Path(p.Scene, global, nextRole.GlobalPos, false, true, () =>
                                {
                                    index++;
                                    operate.context.AddByAgent(nextRole, false);
                                });
                            }
                        }
                        while (index++ < 3)
                        {
                            BuffEx.Set导电Path(p.Scene, global, global + new Vector2Int(Random.Range(-1, 2), Random.Range(-1, 2)) * Random.Range(10, 30), false, false);
                        }
                        p.RemainTime = 0;
                    }
                };
