...
//            public override bool CheckUp() => base.CheckUp() && operate.Agent.GetBuff<Buff_2_11>() != null && operate.Agent.GetBuff<Buff_2_11>().GetActive() == false;
...
//                if (nav.ChackCanNav(operate.Agent.Scene, endGlobal) == false)
//                {
//                    if (nav.ChackCanNav(operate.Agent.Scene, endGlobal + Vector2Int.up)) endGlobal += Vector2Int.up;
//                    else
//                    {
//                        bool canNav = false;
//                        for (int y = -1; y >= -5; y--)
//                            if (nav.ChackCanNav(operate.Agent.Scene, endGlobal + new Vector2Int(0, y)))
//                            {
...
//                    }
//                }
//                nav.TryNav(operate.Agent.Scene, Block.WorldToGlobal(operate.Agent.transform.position), endGlobal, operate.queue, operate.dic, operate.navList);

//                for (int i = 0; i < operate.navList.Count; i++)
//                {
//                    Particle particle = pool.TakeOut(operate.Agent.Scene);
...
//            {
//                base.Enter();
//                operate.Agent.Rig2D.simulated = false;
//                Vector3[] path = new Vector3[operate.navList.Count];
//                for (int i = 0; i < operate.navList.Count; i++)
//                    path[i] = Block.GlobalToWorld(operate.navList[i]);
//                operate.Agent.transform.DOPath(path, 0.1f * operate.navList.Count, PathType.CatmullRom).SetEase(Ease.Linear);
...
//                    operate.Agent.Rig2D.simulated = true;
//                    return TriggerState.toT0;
//                }
//                return TriggerState.update;
//            }
