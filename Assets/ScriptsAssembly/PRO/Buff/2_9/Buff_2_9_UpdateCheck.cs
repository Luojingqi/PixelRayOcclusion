...
namespace PRO.Buff
{
    ...
    {
        private class Buff_2_9_UpdateCheck : RoleBuffUpdateCheckBase<Buff_2_9>
        {
            public Buff_2_9_UpdateCheck(Buff_2_9 buff) : base(buff)
            {
            }
            ...
            public override void Update()
            {
                ...
                for (int y = agent.nav.AgentMould.size.y - 1; y >= 0; y--)
                    for (int x = agent.nav.AgentMould.size.x - 1; x >= 0; x--)
                    {
                        ...
                        if (pixel != null && pixel.typeInfo.typeName == "油")
