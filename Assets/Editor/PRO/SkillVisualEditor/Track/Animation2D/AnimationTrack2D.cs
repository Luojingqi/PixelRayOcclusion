...
namespace PRO.SkillEditor
{
    ...
    internal class AnimationTrack2D : TrackBase
    {
        ...
        protected override bool ForeachSliceDiskToSlice(Slice_DiskBase sliceDisk)
        {
            switch (sliceDisk)
            {
                case AnimationSlice2D_Disk disk: { AddSlice(CreateSlice<AnimationSlice2D>(disk), -1); return true; }
            }
            return false;
        }
        ...
        protected override void DragAssetExit(DragExitedEvent evt, object[] objects)
        {
            for (int i = 0; i < objects.Length; i++)
            {
                ...
                AnimationSlice2D slice = CreateSlice<AnimationSlice2D>(new AnimationSlice2D_Disk());
                slice.Sprite = sprite;
                AddSlice(slice, LocalPointToSliceIndex(evt.localMousePosition.x) + i);
            }
        }
