...
{
    internal class AttackTestSlice2D_FanShaped : AttackTestSlice2DBase
    {
        ...
        private AttackTestSlice2D_FanShaped_Disk diskData => DiskData as AttackTestSlice2D_FanShaped_Disk;

        public override void DrawGizmo(SkillPlayAgent agent)
        {
            Span<Vector2> span = stackalloc Vector2[Count];
            diskData.CreateRayEndPos(span);
        ...
        [LabelText("夹角")]
        [ShowInInspector]
        public float Angle { get => diskData.angle; set => diskData.angle = Mathf.Max(0, value) % 360; }
        [LabelText("密度")]
        [ShowInInspector]
        public float Density
        {
            get => diskData.density;
            set
            {
                diskData.density = Mathf.Min(10, value);
                Count = Mathf.Max(3, Count);
            }
        }
        [LabelText("数量")]
        [ShowInInspector]
        public int Count { get => (int)(diskData.density * diskData.angle); set => diskData.density = Mathf.Max(3, value) / diskData.angle; }
