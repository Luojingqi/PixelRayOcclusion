...
public class BlockManager : MonoBehaviour
{
    ...
    /// <summary>
    /// 用于区块渲染的材质，包含大量显示相关数据，如缓冲区，计算着色器
    /// </summary>
    public static BlockMaterial mat = new BlockMaterial();
    ...
    private void InitBlockPool()
    {
        ...
        renderer.sharedMaterial = mat.ShareMaterial_block;
    ...
    private void InitBackgroundPool()
    {
        ...
        SpriteRenderer renderer = go.AddComponent<SpriteRenderer>();
        renderer.sharedMaterial = mat.ShareMaterial_background;
        go.gameObject.SetActive(false);
    ...
    public void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ThreadPool.QueueUserWorkItem((obj) =>
            {
                ...
                StarManager.Inst.CreateStar(new(52, 48), 32, 10, 1, 10);
                StarManager.Inst.CreateStar(new(52, 48), 33, 10, 1, 10);
                StarManager.Inst.CreateStar(new(140, 48), 33, 10, 1, 10);
                //StarManager.Inst.CreateStar(new(45, 55), 19, 10, 2, 10);
                for (int i = 0; i <= 0; i++)
                {
                    for (int j = 0; j <= 0; j++)
                ...
                BlockManager.Inst.En_Lock_DrawApplyQueue(BlockManager.Inst.BlockCrossList[0][0]);
                BlockManager.Inst.En_Lock_DrawApplyQueue(BlockManager.Inst.BackgroundCrossList[0][0]);
                BlockManager.Inst.En_Lock_DrawApplyQueue(BlockManager.Inst.BlockCrossList[1][0]);
                BlockManager.Inst.En_Lock_DrawApplyQueue(BlockManager.Inst.BackgroundCrossList[1][0]);
                //BlockManager.Inst.En_Lock_DrawApplyQueue(BlockManager.Inst.BlockCrossList[1][0]);
                //    }
                //}
            });
        ...
        {
            mat.allPixelColorInfoToShader[0] = new PixelColorInfoToShader() { color = new Vector4(0, 1, 0, 1) };
            mat.allPixelColorInfoBufffer.SetData(mat.allPixelColorInfoToShader);
        }
        if (Input.GetKeyDown(KeyCode.Mouse0))
        {
            mat.SetSun(new(Block.WorldToGloab(m), 0, new Vector4(1, 1, 1, 1)), kk++);
            if (kk >= 10) kk = 0;
        }
        ...
        if (Monitor.TryEnter(DrawApplyQueue))
        {
            try
            {
                while (DrawApplyQueue.Count > 0)// for (int i = 0; i < DrawApplyQueue.Count; i++)
                {
                    BlockBase blockBase = DrawApplyQueue.Dequeue();
                    blockBase.spriteRenderer.sprite.texture.Apply();
                    switch (blockBase)
                    {
                        case Block block: mat.SetBlock(block); break;
                        case BackgroundBlock background: mat.SetBackgroundBlock(background); break;
                    }
                }
            }
            finally
    ...
    /// </summary>
    public readonly Queue<DrawGraph.DrawGraphTaskData> DrawGraphTaskQueue = new Queue<DrawGraph.DrawGraphTaskData>();
    /// <summary>
    ...
    public async void EnqueueDrawGraphTaskAsync(DrawGraph.DrawGraphTaskData drawTask)
    {
        while (true)
        {
