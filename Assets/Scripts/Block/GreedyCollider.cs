    ...
    public static List<ColliderData> CreateColliderDataList(Block block, Vector2Byte min, Vector2Byte max)
    {
        ...
        for (int y = max.y; y >= min.y; y--)
            for (int x = min.x; x <= max.x; x++)
                if (!hash.Contains(new Vector2Int(x, y)) && block.GetPixelRelocation(x, y).id != 0)
                {
                    ...
                    int xShifting = x + 1;
                    while (xShifting <= max.x && !hash.Contains(new Vector2Int(xShifting, y)) && block.GetPixelRelocation(xShifting, y).id != 0)
                    ...
                    while (ok && yShifting >= min.y)
                    {
                        ...
                        {
                            if (!hash.Contains(new Vector2Int(x + i, yShifting)) && block.GetPixelRelocation(x + i, yShifting).id != 1)
                            {
                                ok = false;
                                break;
                            }
